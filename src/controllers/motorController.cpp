  ...
  while (true) {
    ...
    if (xSemaphoreTake(xMotorControllerSemaphore, pdMS_TO_TICKS(10) ) == pdTRUE ) { 
      if ( this->status == CONTROLLER_ON ){
        ...
        double delta_angle = current_position - prev_angle;
...
MotorController::MotorController(const char* description, SensorType_t sensor_type, MotorType_t motor_type ) 
  : description( description ), sensor_type( sensor_type), motor_type( motor_type ) {
    ...
    xTaskCreate( [](void *arg){Instance->step(nullptr);}, "stepControllTask", 2048, NULL, 1, stepControllTask );
